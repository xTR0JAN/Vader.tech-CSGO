...
void Form::draw() {
	...
	// background.
	render::rect_filled(m_x, m_y, m_width, m_height, { 17, 17, 17, m_alpha });
	...
	std::string text = tfm::format(XOR("rawrwebz | %s | %s"), date.c_str(), time.c_str());
	render::menu_shade.string(m_x + 3, m_y - 15, { 255, 255, 255, m_alpha }, text);
	...
	if (!m_tabs.empty()) {
		...
		render::rect(tabs_area.x + 1, tabs_area.y + 1, tabs_area.w - 2, tabs_area.h - 2, { 25, 26, 33, m_alpha });

		for (size_t i{}; i < m_tabs.size(); ++i) {
			const auto& t = m_tabs[i];

			render::menu.string(tabs_area.x + 10, tabs_area.y + 5 + (i * 16),
				t == m_active_tab ? color : Color{ 152, 152, 152, m_alpha },
				t->m_title);

			render::rect_filled(tabs_area.x, tabs_area.y + 5 + (i * 16), 2, 12, t == m_active_tab ? color : Color(0, 0, 0, 0) );
		}
		...
		if (!m_active_tab->m_elements.empty()) {
			...
			render::rect(el.x + 1, el.y + 1, el.w - 2, el.h - 2, { 25, 26, 33, m_alpha });
