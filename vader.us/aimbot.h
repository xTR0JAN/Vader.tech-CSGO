...
struct HitscanData_t {
	...
	__forceinline HitscanData_t() : m_damage{ 0.f }, m_pos{ }, m_hitbox{ }, m_mode{} {}
};
...
public:
	...
	void think();
	void find();
	bool can_hit_hitbox(const vec3_t start, const vec3_t end, LagRecord* animation, int box, bool in_shot = false, BoneArray* bones = nullptr);
	bool CanHit(vec3_t start, vec3_t end, LagRecord* record, int box, bool in_shot, BoneArray* bones);
	bool CheckHitchance(Player* player, const ang_t& angle, float chance);
