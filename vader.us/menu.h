...
public:
	...
	Slider        debugaim_alpha;
	Checkbox      debug_tools;
	...
	Checkbox      norecoil;
	Checkbox      hitchance;
	Checkbox	  hitchance_noscope;
	Slider	      hitchance_amount;
	Slider	      hitchance_noscope_amount;
	...
	Keybind       override;
	Keybind       doubletap;
	Slider        doubletap_ticks;
	Checkbox      slow_teleport;
...
public:
	void init() {
		...
		debugaim.setup(XOR("draw debug aimbot"), XOR("debugaim"));
		RegisterElement(&debugaim);

		debugaim_color.setup("color", XOR("debugaim_color"), colors::white);
		RegisterElement(&debugaim_color);

		debugaim_alpha.setup(XOR(""), XOR("debugaim_alpha"), 0.f, 255.f, false, 0, 150.f, 1.f, XOR(L"°"));
		RegisterElement(&debugaim_alpha);

		debug_tools.setup(XOR("draw debug tools"), XOR("debug_tools"));
		RegisterElement(&debug_tools);
		...
		hitchance_noscope.setup(XOR("no scope hitchance"), XOR("hitchance_noscope"));
		hitchance_noscope.AddShowCallback(callbacks::IsConfigMM);
		RegisterElement(&hitchance_noscope, 1);

		hitchance_noscope_amount.setup("", XOR("hitchance_noscope_amount"), 1.f, 100.f, false, 0, 50.f, 1.f, XOR(L"%"));
		hitchance_noscope_amount.AddShowCallback(callbacks::IsConfigMM);
		hitchance_noscope_amount.AddShowCallback(callbacks::IsHitchanceNoscopeOn);
		RegisterElement(&hitchance_noscope_amount, 1);
		...
		min_key.setup(XOR("override minimum damage key"), XOR("min_key"));
		RegisterElement(&min_key, 1);

		doubletap_ticks.setup(XOR("doubletap ticks"), XOR("doubletap_ticks"), 8.f, 17.f, false, 0, 14.f, 1.f, XOR(L"°"));
		RegisterElement(&doubletap_ticks, 1);

		slow_teleport.setup(XOR("extend teleport"), XOR("slow_teleport"));
		RegisterElement(&slow_teleport, 1);
	}
};
...
public:
	void init() {
		...
		// stand.
		pitch_stand.setup(XOR("pitch"), XOR("pitch_stnd"), { XOR("off"), XOR("down"), XOR("up"), XOR("random"), XOR("ideal"), XOR("fake up"), XOR("fake down") });
		...
		amongus_aa.setup(XOR("among us"), XOR("amongus_aa"));
		RegisterElement(&amongus_aa);
		...
		body_fake_stand.setup(XOR("fake body"), XOR("body_fake_stnd"), { XOR("off"), XOR("left"), XOR("right"), XOR("opposite"), XOR("z"), XOR("custom"), XOR("twist"), XOR("desync flick") });
		...
		suppress.setup(XOR("suppress activity 979 (op)"), XOR("supress"));
		RegisterElement(&suppress);
		...
		// walk.
		pitch_walk.setup(XOR("pitch"), XOR("pitch_walk"), { XOR("off"), XOR("down"), XOR("up"), XOR("random"), XOR("ideal"), XOR("fake up"), XOR("fake down") });
		...
		// air.
		pitch_air.setup(XOR("pitch"), XOR("pitch_air"), { XOR("off"), XOR("down"), XOR("up"), XOR("random"), XOR("ideal"), XOR("fake up"), XOR("fake down") });
		...
		// col2.
		fake_yaw.setup(XOR("fake yaw"), XOR("fake_yaw"), { XOR("off"), XOR("default"), XOR("relative"), XOR("jitter"), XOR("rotate"), XOR("random"), XOR("ballerina") });
		...
		manul_antiaim.setup(XOR("show manual anti-aim"), XOR("manul_antiaim"));
		...
		aaTest.setup(XOR("fake flick"), XOR("aaTest"));
		aaTest.SetToggleCallback(callbacks::ToggleAATest);
		RegisterElement(&aaTest);

		angleflick.setup("", XOR("angleflick"), -180.f, 180.f, false, 0, 145.f, 5.f, XOR(L"°"));
		RegisterElement(&angleflick);
...
public:
	...
	Colorpicker   ammo_color_g;
	Checkbox      lby_update;
	Colorpicker   lby_update_color;
	Colorpicker   lby_update_color_g;
...
public:
	void init() {
		...
		gradient_esp.setup(XOR("gradient esp"), XOR("gradient_esp"));
		RegisterElement(&gradient_esp);
		...
		health_color.setup(XOR("health color"), XOR("health_color"), Color(0, 255, 0));
		RegisterElement(&health_color);

		health_color_g.setup(XOR("health color gradient"), XOR("health_color_g"), Color(200, 0, 0));
		health_color_g.AddShowCallback(callbacks::IsGradientESP);
		RegisterElement(&health_color_g);

		flags_enemy.setup(XOR("flags enemy"), XOR("flags_enemy"), { XOR("money"), XOR("armor"), XOR("scoped"), XOR("flashed"), XOR("reloading"), XOR("bomb"), XOR("fake"), XOR("broke lc"), XOR("resolver") });
		...
		ammo_color_g.setup(XOR("color gradient"), XOR("ammo_color_g"), Color(0,0,150));
		ammo_color_g.AddShowCallback(callbacks::IsGradientESP);
		RegisterElement(&ammo_color_g);
		...
		lby_update_color.setup(XOR("color"), XOR("lby_update_color"), Color(230, 0, 255));
		RegisterElement(&lby_update_color, 1);

		lby_update_color_g.setup(XOR("color gradient"), XOR("lby_update_color_g"), Color(50, 0, 200));
		lby_update_color_g.AddShowCallback(callbacks::IsGradientESP);
		RegisterElement(&lby_update_color_g, 1);
		...
		chams_local_mat.setup(XOR("chams local material"), XOR("chams_local_mat"), { XOR("material"), XOR("flat"), XOR("metallic"), XOR("shaded"), XOR("glow"), XOR("stick man") });
...
public:
	void init() {
		...
		world_color.setup(XOR("color"), XOR("world_color"), colors::white);
		world_color.SetCallback(Visuals::ModulateWorld);
		world_color.AddShowCallback(callbacks::IsCustomWorld);
...
public:
	...
	Keybind  fakewalk;
	Keybind  nefariouswalk;
	Slider   seanjog_safety;
	Keybind  seannigger;
	Keybind  serverside;
	Keybind  autopeek;
	Keybind  autostop;
...
public:
	void init() {
		...
		seannigger.setup(XOR("seannigger"), XOR("seannigger"));
		RegisterElement(&seannigger, 1);

		nefariouswalk.setup(XOR("sean jog"), XOR("nefariouswalk"));
		RegisterElement(&nefariouswalk, 1);

		seanjog_safety.setup("safety", XOR("seanjog_safety"), 0.f, 6.f, true, 0, 2.f, 1.f, XOR(L"%"));
		RegisterElement(&seanjog_safety, 1);
...
public:
	void init() {
		...
		// setup models here
		agent_change.setup(XOR("Setup agent"), XOR("agent_changer"), { XOR("mamogus"),  XOR("stormtrooper"), XOR("darth vader"), XOR("shrek") });
		RegisterElement(&agent_change);

		ForceUpdate_agent.setup(XOR("Force update"));
		ForceUpdate_agent.SetCallback(callbacks::ForceFullUpdate);
		RegisterElement(&ForceUpdate_agent);

		enable.setup(XOR("enable"), XOR("skins_enable"));
		enable.SetCallback(callbacks::ForceFullUpdate);
		RegisterElement(&enable, 1);
...
public:
	void init() {
		...
		ihadtololsorry.setup(XOR("godly feature"), XOR("godlyfeaturexdddddd"));
		RegisterElement(&ihadtololsorry);
		...
		aspect_ratio_amt.setup(XOR(""), XOR("aspect_ratio_amt"), 0.f, 2.f, false, 1, 0.f, 0.1f, XOR(L"°"));
		RegisterElement(&aspect_ratio_amt);
		...
		hitsounds.setup(XOR("hitsound"), XOR("hitsounds"), { XOR("off"), XOR("arena switch"), XOR("opera"), XOR("bameware"), XOR("dopium"), XOR("bubble"), XOR("cod"), XOR("fatality"), XOR("pop"), XOR("neverlose")});
		RegisterElement(&hitsounds, 1);

		killsound.setup(XOR("killsound"), XOR("killsound"));
		RegisterElement(&killsound, 1);
...
public:
	...
	void init() {
		...
		server_selection.setup(XOR("server selection"), XOR("serer_selection"), { "emporium", "dignity", "vutra" });
		RegisterElement(&server_selection);

		server_join.setup(XOR("join server"));
		server_join.SetCallback(callbacks::JoinServer);
		RegisterElement(&server_join);
