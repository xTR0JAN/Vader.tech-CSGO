...
// setup models
void Skins::SetupAgents()
{
	//if (!ran) {
	//	ran = true;
		// call this once
		if (!g_menu.main.skins.enable_agent.get())
		{
			// Strip?
			// TODO backup and save default model

			if (g_menu.main.skins.agent_change.get() == 0)	// first
				g_csgo.m_model_info->GetModelIndex("models/player/custom_player/owston/amongus/white.mdl");
			else if (g_menu.main.skins.agent_change.get() == 1)
				g_csgo.m_model_info->GetModelIndex("models/player/custom_player/kuristaja/stormtrooper/stormtrooper.mdl");
			else if (g_menu.main.skins.agent_change.get() == 2)
				g_csgo.m_model_info->GetModelIndex("models/player/custom_player/kuristaja/vader/vader.mdl");
			else if (g_menu.main.skins.agent_change.get() == 3)
				g_csgo.m_model_info->GetModelIndex("models/player/custom_player/kuristaja/shrek/shrek.mdl");
			ForceOnce = true;
			LoadModel = false;
			return;
		}
		...
		if (ForceOnce == false)
		{
			...
			static int iModel = g_menu.main.skins.agent_change.get();
