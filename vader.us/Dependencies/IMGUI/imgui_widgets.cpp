...
bool ImGui::TitleButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
...
bool ImGui::TitleButton(const char* label, const ImVec2& size_arg)
{
	return TitleButtonEx(label, size_arg, 0);
}
...
bool ImGui::SelectedSubTabEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::SubTab(const char* label, const ImVec2& size_arg)
{
	return SubTabEx(label, size_arg, 0);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (window->DC.ItemFlags & ImGuiItemFlags_MixedValue)
	{
		...
		const float pad2 = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));

		if (hovered)
			RenderCheckMark(check_bb.Min + ImVec2(pad2, pad2), ImColor(61, 59, 67), square_sz - pad2 * 2.0f);
	}
	else if (*v)
	{
		...
		if (hovered) {
			...
			RenderCheckMark(check_bb.Min + ImVec2(pad, pad), ImColor(91, 89, 97), square_sz - pad * 2.0f);
		}
		else if (IsItemActive()) {

			RenderCheckMark(check_bb.Min + ImVec2(pad, pad), ImColor(101, 99, 107), square_sz - pad * 2.0f);
		}
		else
			RenderCheckMark(check_bb.Min + ImVec2(pad, pad), ImColor(81, 79, 87), square_sz - pad * 2.0f);
	}
...
bool ImGui::Hotkey(const char* label, int* variable, const ImVec2& getSize) {
	...
	const ImGuiID id = window->GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	ImVec2 size = CalcItemSize(getSize, CalcItemWidth(), label_size.y + style.FramePadding.y*2.0f);
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(label_size.x + style.ItemInnerSpacing.x - 23.f, 0.0f), window->DC.CursorPos + size);
	...
	auto bottomColor = ImColor(31, 31, 31, 255);
	auto topColorHovered = ImColor(41, 41, 41, 255);
	auto bottomColorHovered = ImColor(41, 41, 41, 255);
	...
	if (*variable != 0 && g.ActiveId != id) {
		strcpy(buf_display, keyNames[*variable]);
	}
	...
	RenderTextClipped(frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding, buf_display, NULL, NULL, ImVec2(0.5f, 0.4f));

	//if (label_size.x > 0)
	//	RenderText(ImVec2(total_bb.Max.x, frame_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	if (hovered || popup_open) {
		...
		window->DrawList->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max, topColorHovered, topColorHovered, bottomColorHovered, bottomColorHovered);
	}
	else {

		window->DrawList->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max, topColor, topColor, bottomColor, bottomColor);
	}
	RenderFrameBorder(frame_bb.Min, frame_bb.Max, style.FrameRounding);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	...
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	ImGuiStyle* cockstyle = &ImGui::GetStyle();
	...
	if (tab_focus_requested || (hovered && g.IO.MouseClicked[0]) || g.NavActivateId == id || (g.NavInputId == id && g.TempInputTextId != id)) {
	...
	auto bottomColor = ImColor(52, 52, 52, 255);
	auto topColorHovered = ImColor(62, 62, 62, 255);
	auto bottomColorHovered = ImColor(62, 62, 62, 255);
	auto grabTopColor = GetColorU32(ImGuiCol_MenuAccent);

	if (hovered || IsItemActive())
		window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(0, 0.5), frame_bb.Max, topColorHovered, topColorHovered, bottomColorHovered, bottomColorHovered);
	else
		window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(0, 0.5), frame_bb.Max, topColor, topColor, bottomColor, bottomColor);

	window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(0, 0.5), grab_bb.Max + ImVec2(1, 1), grabTopColor, grabTopColor, grabTopColor, grabTopColor);
	window->DrawList->AddRect(frame_bb.Min + ImVec2(0, 0.5), frame_bb.Max, borderColor, 0, false, 0.4);

	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);


	//cockstyle->Colors[ImGuiCol_Text] = ImColor(0, 0, 0);
	//RenderTextClipped(ImVec2(grab_bb.Min.x - 75, grab_bb.Max.y - 12), ImVec2(grab_bb.Max.x + 101, grab_bb.Max.y + 8), value_buf, value_buf_end, NULL, ImVec2(0.51f, 0.5f));
	//cockstyle->Colors[ImGuiCol_Text] = ImColor(255, 255, 255);

	//RenderTextClipped(ImVec2(grab_bb.Min.x - 75, grab_bb.Max.y - 12), ImVec2(grab_bb.Max.x + 101, grab_bb.Max.y + 8), value_buf, value_buf_end, NULL, ImVec2(0.51f, 0.5f));

	//cockstyle->Colors[ImGuiCol_Text] = ImColor(0, 0, 0);

	/*float x = 120 + (strlen(value_buf) * 2), y = 12;*/

	//ImGui::PopFont();
	//ImGui::PushFont(gravity);

	//RenderTextClipped(ImVec2(x - 2, y - 1), ImVec2(grab_bb.Max.x + 101, grab_bb.Max.y + 8), value_buf, value_buf_end, NULL, ImVec2(0.51f, 0.5f));
	...
	//
	//RenderTextClipped(ImVec2(x - 2, y + 1), ImVec2(grab_bb.Max.x + 101, grab_bb.Max.y + 6), value_buf, value_buf_end, NULL, ImVec2(0.51f, 0.5f));
	//
	...
	//
	//RenderTextClipped(ImVec2(x, y), ImVec2(grab_bb.Max.x + 101, grab_bb.Max.y + 8), value_buf, value_buf_end, NULL, ImVec2(0.51f, 0.5f));

	//RenderTextClipped(ImVec2(grab_bb.Min.x - x - 2, grab_bb.Max.y - y - 1), ImVec2(grab_bb.Max.x + 101, grab_bb.Max.y + 8), value_buf, value_buf_end, NULL, ImVec2(0.51f, 0.5f));
	...
	//
	//RenderTextClipped(ImVec2(grab_bb.Min.x - x - 2, grab_bb.Max.y - y + 1), ImVec2(grab_bb.Max.x + 101, grab_bb.Max.y + 6), value_buf, value_buf_end, NULL, ImVec2(0.51f, 0.5f));
	//
	...
	//
	//RenderTextClipped(ImVec2(grab_bb.Min.x - x, grab_bb.Max.y - y), ImVec2(grab_bb.Max.x + 101, grab_bb.Max.y + 8), value_buf, value_buf_end, NULL, ImVec2(0.51f, 0.5f));
	...
	if (label_size.x > 0) {
		...
		RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 163, frame_bb.Min.y + style.FramePadding.y - 18), label);
		//RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 50, frame_bb.Min.y + style.FramePadding.y - 18), "-");
		RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - (11 + (strlen(value_buf) * 5)), frame_bb.Min.y + style.FramePadding.y - 18), value_buf);
	}
