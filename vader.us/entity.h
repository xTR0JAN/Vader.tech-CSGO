...
public:
	...
	PAD(0x4);					// 0x0340
	float* timeinair() {
		return reinterpret_cast<float*>((DWORD)this + 0x110);
	}
	float* m_flHeadHeightOrOffsetFromHittingGroundAnimation() { //0x118 from 0 to 1, is 1 when standing
		return reinterpret_cast<float*>((DWORD)this + 0x118);
	}
	float* m_flStopToFullRunningFraction() { //0x11C from 0 to 1, doesnt change when walking or crouching, only running
		return reinterpret_cast<float*>((DWORD)this + 0x11C);
	}
	float* m_flLastOriginZ() { //0x11C from 0 to 1, doesnt change when walking or crouching, only running
		return reinterpret_cast<float*>((DWORD)this + 0x114);
	}
