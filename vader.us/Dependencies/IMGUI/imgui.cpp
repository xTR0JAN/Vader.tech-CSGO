...
namespace ImGui
{
	...
	static bool             BeginTitleBarEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags);
	...
	static void             RenderTitleBarBackgroundDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size);
...
// custom title bar
static bool ImGui::BeginTitleBarEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* parent_window = g.CurrentWindow;
	...
	if (!border)
		g.Style.ChildBorderSize = 0.0f;
	bool ret = BeginTitleBarBackground(title, NULL, flags);
	g.Style.ChildBorderSize = backup_border_size;
...
bool ImGui::BeginTitleBar(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	return BeginTitleBarEx(str_id, window->GetID(str_id), size_arg, border, extra_flags);
}

bool ImGui::BeginTitleBar(ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	IM_ASSERT(id != 0);
	return BeginTitleBarEx(NULL, id, size_arg, border, extra_flags);
}
...
bool ImGui::CheckBoxAnimated(const char* label, bool* v, int animstep) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	ImColor background = GetColorU32(ImGuiCol_MenuAccent);

	if (!strcmp(label, "##"))
	{
		alpha = 0;
		background = ImColor(0, 0, 0, 0);
	}

	ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 1, label_size.y + style.FramePadding.y * 1)); // We want a square shape to we use Y twice		
	...
	if (*v && animstep > 0 && animstep < 71) {
		if (animstep <= 35) {
			float step = 3.f / 35.f;
			float width = step * animstep;
			check_bb = ImRect(check_bb.Min.x + width, check_bb.Min.y + width, check_bb.Max.x - width, check_bb.Max.y - width);
		}
		else {
			float step = 3.f / 35.f;
			float width = 3.f - (step * (animstep - 35));
			check_bb = ImRect(check_bb.Min.x + width, check_bb.Min.y + width, check_bb.Max.x - width, check_bb.Max.y - width);
		}
	}

	//RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
	window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, (!animstep ? ImColor(119, 119, 119, alpha) : background), 2);

	ImRect inner_bb(check_bb.Min.x + 2.f, check_bb.Min.y + 2.f, check_bb.Max.x - 2.f, check_bb.Max.y - 2.f);

	if (animstep > 0 && animstep < 100) {

		float step = inner_bb.GetWidth() / 100.f;
		float sub = step * animstep;

		inner_bb = ImRect(inner_bb.Min.x + sub / 2.f, inner_bb.Min.y + sub / 2.f, inner_bb.Max.x - sub / 2.f, inner_bb.Max.y - sub / 2.f);

	}

	if (animstep < 100) {
		window->DrawList->AddRectFilled(inner_bb.Min, inner_bb.Max, ImColor(21, 17, 29, alpha));
	}
	else {
		const float check_sz = ImMin(inner_bb.GetWidth(), inner_bb.GetHeight());
		const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
		RenderCheckMark(inner_bb.Min + ImVec2(pad, pad), ImColor(34, 34, 34, alpha), inner_bb.GetWidth() - pad * 2.0f);

		if (animstep < 200) {
			float step = inner_bb.GetWidth() / 100.f;
			float sub = step * (animstep - 100);
			ImRect over_bb = ImRect(inner_bb.Min.x + sub, inner_bb.Min.y, inner_bb.Max.x, inner_bb.Max.y);
			window->DrawList->AddRectFilled(over_bb.Min, over_bb.Max, background);
		}
	}
...
{
	return ImMax(0.0f, window->ContentSize.y - (window->SizeFull.y - window->ScrollbarSizes.y));
}
...
void ImGui::RenderTitleBarBackgroundDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
	ImGuiContext& g = *GImGui;
	ImGuiStyle& style = g.Style;
	ImGuiWindowFlags flags = window->Flags;
	...
	{
		// Window background
		window->DrawList->AddRectFilled(window->Pos + ImVec2(0.f, window->TitleBarHeight() + 0.f), window->Pos + window->Size + ImVec2(0.f, 0.f), ImColor(31, 24, 46), 0.f);

		window->DrawList->AddRectFilled(window->Pos + ImVec2(0.f, window->TitleBarHeight() + 54.f), window->Pos + window->Size, GetColorU32(ImGuiCol_Logo), 0.f);
...
bool ImGui::BeginTitleBarBackground(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	IM_ASSERT(name != NULL && name[0] != '\0');     // Window name required
	...
	if (first_begin_of_the_frame)
	{
		...
		const ImGuiWindow* window_to_highlight = g.NavWindowingTarget ? g.NavWindowingTarget : g.NavWindow;
		const bool title_bar_is_highlight = want_focus || (window_to_highlight && window->RootWindowForTitleBarHighlight == window_to_highlight->RootWindowForTitleBarHighlight);
		RenderTitleBarBackgroundDecorations(window, title_bar_rect, title_bar_is_highlight, resize_grip_count, resize_grip_col, resize_grip_draw_size);
...
void ImGui::PushStyleColor(ImGuiCol idx, ImGuiCol idx2) {
